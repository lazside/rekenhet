...
export default function LicensePlateInput() {
  ...
  return (<div className="space-y-6">
    ...
    <div className="rounded-xl border-2 border-gray-300 bg-white p-5 shadow-sm">
      <div className="flex items-center gap-2 mb-3"><Car className="h-5 w-5 text-indigo-600"/><h2 className="text-sm font-semibold text-gray-900">Kenteken check</h2></div>
      ...
      <button onClick={handleSearch} disabled={loading||!plate.trim()} className="flex shrink-0 items-center gap-2 rounded-lg bg-indigo-600 px-5 py-2 text-sm font-semibold text-white hover:bg-indigo-700 disabled:opacity-50 transition-all">{loading?<Loader2 className="h-4 w-4 animate-spin"/>:<Search className="h-4 w-4"/>}Check</button></div>
    ...
    {result&&(<>
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
        <div className="flex items-start gap-3 mb-4">
          <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-xl bg-blue-100"><Car className="h-7 w-7 text-indigo-600"/></div>
          ...
          <button onClick={handleShare} className={cn("flex h-8 w-8 items-center justify-center rounded-lg transition-all",copied?"bg-emerald-100 text-emerald-600":"bg-gray-100 text-gray-400 hover:bg-blue-50 hover:text-indigo-600")}aria-label="Delen">{copied?<Check className="h-4 w-4"/>:<Share2 className="h-4 w-4"/>}</button>
        </div>
      ...
      {/* Jaarlijkse kosten */}
      <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
        <h3 className="text-sm font-semibold text-gray-900 mb-3 flex items-center gap-2"><Euro className="h-4 w-4 text-indigo-600"/>Jaarlijkse kosten</h3>
        <div className="rounded-xl bg-gradient-to-br from-indigo-500 to-indigo-700 p-5 text-white shadow-lg text-center mb-4"><p className="text-sm text-indigo-200">Totale geschatte jaarkosten</p><p className="text-3xl font-bold tabular-nums mt-1">€{totaalPerJaar.toLocaleString("nl-NL")}</p><p className="text-xs text-indigo-300 mt-1">€{Math.round(totaalPerJaar/12).toLocaleString("nl-NL")}/mnd</p></div>
        <div className="grid grid-cols-3 gap-3">
          ...
          <div className="rounded-lg bg-blue-50 p-3 text-center border border-blue-100"><p className="text-[10px] text-indigo-600">Brandstof/jaar</p><p className="text-sm font-bold text-blue-700 tabular-nums">{result.data.isEv?"€0 (EV)":`€${brandstofKostenPerJaar.toLocaleString("nl-NL")}`}</p></div>
        ...
        ].map(inp=>(<div key={inp.l} className="space-y-1"><label className="text-xs text-gray-500">{inp.l}: {typeof inp.v==="number"&&inp.step<1?`€${inp.v.toFixed(2)}`:inp.v.toLocaleString("nl-NL")}</label><input type="range" min={0} max={inp.max} step={inp.step} value={inp.v} onChange={e=>inp.s(+e.target.value)} className="w-full accent-indigo-600"/></div>))}</div>}
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {/* MRB */}
        <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
          <h3 className="text-sm font-semibold text-gray-900 mb-3 flex items-center gap-2"><Coins className="h-4 w-4 text-indigo-600"/>Wegenbelasting</h3>
          <div className="flex flex-wrap gap-1.5 mb-3">{Object.entries(provLabels).slice(0,6).map(([k,v])=>(<button key={k} onClick={()=>setSelProv(k)} className={cn("rounded-lg px-2.5 py-1 text-[11px] font-medium transition-all",selProv===k?"bg-indigo-600 text-white":"bg-gray-100 text-gray-600 hover:bg-gray-200")}>{v}</button>))}</div>
        ...
        <div className="space-y-4">
          ...
          {/* CO2 vergelijking */}
          <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
            <h3 className="text-sm font-semibold text-gray-900 mb-3 flex items-center gap-2"><Fuel className="h-4 w-4 text-indigo-600"/>CO₂-uitstoot</h3>
      ...
      {/* Bijtelling */}
      {result.bijtelling&&<div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm"><h3 className="text-sm font-semibold text-gray-900 mb-3 flex items-center gap-2"><Euro className="h-4 w-4 text-indigo-600"/>Bijtelling (zakelijk)</h3><div className="grid grid-cols-2 gap-3"><div className="rounded-xl bg-gradient-to-br from-indigo-500 to-indigo-700 p-4 text-white text-center"><p className="text-xs text-indigo-200">Per maand</p><p className="text-2xl font-bold tabular-nums mt-1">€{result.bijtelling.bijtellingPerMaand}</p></div><div className="rounded-xl bg-gray-50 p-4 text-center"><p className="text-xs text-gray-500">Percentage</p><p className="text-2xl font-bold text-gray-900 tabular-nums mt-1">{result.bijtelling.percentage}%</p></div></div></div>}

      {/* Affiliate CTA */}
      <a href="/go/verzekering-vergelijken" target="_blank" rel="noopener noreferrer sponsored" className="flex items-center justify-between rounded-xl bg-gradient-to-r from-indigo-600 to-indigo-700 p-5 text-white shadow-lg hover:shadow-xl transition-all group"><div><p className="font-semibold">Besparen op de autoverzekering?</p><p className="text-sm text-indigo-200 mt-0.5">Vergelijk direct de premies voor deze {result.data.merk}.</p></div><ArrowRight className="h-5 w-5 shrink-0 transition-transform group-hover:translate-x-1"/></a>
    </>)}
  </div>);
}
