...
export default function TripCostCalculator() {
  ...
  return (<div className="space-y-6"><div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm space-y-5">
    <h2 className="text-sm font-semibold text-gray-900 flex items-center gap-2"><Car className="h-4 w-4 text-indigo-600" />Ritkosten</h2>
    <div className="grid grid-cols-2 gap-4">
      <div className="space-y-1.5"><label className="text-sm font-medium text-gray-700">Afstand (km)</label><input type="text" value={d} onChange={e => setD(+e.target.value||0)} className="input-base"/></div>
      <div className="space-y-1.5"><label className="text-sm font-medium text-gray-700">Verbruik (km/l)</label><input type="text" value={c} onChange={e => setC(+e.target.value||0)} className="input-base"/></div>
      <div className="space-y-1.5"><label className="text-sm font-medium text-gray-700">Brandstofprijs (€/l)</label><input type="text" step="0.01" value={p} onChange={e => setP(+e.target.value||0)} className="input-base"/></div>
      <div className="space-y-1.5"><label className="text-sm font-medium text-gray-700">Tol (€)</label><input type="text" value={t} onChange={e => setT(+e.target.value||0)} className="input-base"/></div>
    </div>
    <div className="space-y-1.5"><label className="text-sm font-medium text-gray-700">Parkeren (€)</label><input type="text" value={pk} onChange={e => setPk(+e.target.value||0)} className="input-base"/></div>
  </div>
  <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm space-y-3">
    <h2 className="text-sm font-semibold text-gray-900 flex items-center gap-2"><Euro className="h-4 w-4 text-indigo-600" />Kostenoverzicht</h2>
