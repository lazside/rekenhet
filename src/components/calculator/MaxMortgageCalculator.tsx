...
export default function MaxMortgageCalculator() {
  ...
  return (<div className="space-y-6"><div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm space-y-5">
    <h2 className="text-sm font-semibold text-gray-900 flex items-center gap-2"><Home className="h-4 w-4 text-indigo-600" />Maximale Hypotheek</h2>
    <div className="grid grid-cols-2 gap-4">
      <div className="space-y-1.5"><label className="text-sm font-medium text-gray-700">Jouw inkomen (€)</label><input type="text" value={i} onChange={e=>setI(+e.target.value||0)} className="input-base"/></div>
      <div className="space-y-1.5"><label className="text-sm font-medium text-gray-700">Partnerinkomen (€)</label><input type="text" value={pi} onChange={e=>setPi(+e.target.value||0)} className="input-base"/></div>
      <div className="space-y-1.5"><label className="text-sm font-medium text-gray-700">Rente (%)</label><input type="text" step="0.1" value={r} onChange={e=>setR(+e.target.value||0)} className="input-base"/></div>
      <div className="space-y-1.5"><label className="text-sm font-medium text-gray-700">Looptijd (jr)</label><input type="text" value={l} onChange={e=>setL(+e.target.value||0)} className="input-base"/></div>
    </div>
  </div>
  <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm space-y-3">
    <div className="rounded-xl bg-gradient-to-br from-indigo-600 to-indigo-800 p-5 text-white shadow-lg text-center"><p className="text-sm text-indigo-200">Maximale hypotheek</p><p className="text-3xl font-bold tabular-nums mt-1">{formatEUR(res.maxHypotheek)}</p></div>
