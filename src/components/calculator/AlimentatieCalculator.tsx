...
export default function AlimentatieCalculator() {
  ...
  return (<div className="space-y-6">
    <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4"><div className="space-y-1.5"><label className="text-sm font-medium text-gray-700">Netto inkomen ouder 1 (€)</label><input type="number" value={i1} onChange={e=>setI1(+e.target.value||0)} className="w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm"/></div><div className="space-y-1.5"><label className="text-sm font-medium text-gray-700">Netto inkomen ouder 2 (€)</label><input type="number" value={i2} onChange={e=>setI2(+e.target.value||0)} className="w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm"/></div></div>
      <div className="space-y-1.5"><label className="text-sm font-medium text-gray-700">Aantal kinderen</label><input type="number" min={1} value={ak} onChange={e=>setAk(Math.max(1,+e.target.value||1))} className="w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm"/></div>
    </div>
    <div className="grid grid-cols-2 gap-3">
      ...
      <div className="rounded-xl bg-blue-500 bg-gradient-to-br to-indigo-700 p-4 text-white shadow-sm text-center"><p className="text-xs text-blue-200">Ouder 2 bijdrage</p><p className="text-2xl font-bold tabular-nums mt-1">{formatEUR(r.ouder2Bijdrage)}/mnd</p></div>
    </div>
