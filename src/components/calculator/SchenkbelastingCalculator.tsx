...
export default function SchenkbelastingCalculator() {
  ...
  return (<div className="space-y-6">
    <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4"><div className="space-y-1.5"><label className="text-sm font-medium text-gray-700">Bedrag (€)</label><input type="text" value={b} onChange={e=>setB(+e.target.value||0)} className="input-base"/></div><div className="space-y-1.5"><label className="text-sm font-medium text-gray-700">Relatie</label><select value={g} onChange={e=>setG(e.target.value as Tariefgroep)} className="input-base">{(Object.entries(TARIEF_LABELS) as [Tariefgroep, string][]).map(([k,v])=><option key={k} value={k}>{v}</option>)}</select></div></div>
      <div className="flex gap-2"><button onClick={()=>setE(false)} className={`flex-1 rounded-lg px-3 py-2.5 text-sm font-medium transition-all ${!e?"bg-indigo-600 text-white shadow-sm":"bg-gray-100 text-gray-600"}`}>Schenking</button><button onClick={()=>setE(true)} className={`flex-1 rounded-lg px-3 py-2.5 text-sm font-medium transition-all ${e?"bg-indigo-600 text-white shadow-sm":"bg-gray-100 text-gray-600"}`}>Erfenis</button></div>
    </div>
    <div className={`rounded-xl border p-5 shadow-sm space-y-3 ${GROEP_KLEUREN[g]}`}>
      <div className="rounded-xl bg-gradient-to-br from-indigo-600 to-indigo-800 p-5 text-white shadow-lg text-center"><p className="text-sm text-indigo-200">Te betalen belasting</p><p className="text-3xl font-bold tabular-nums mt-1">{formatEUR(r.belasting)}</p><p className="text-xs text-indigo-300 mt-1">Effectief tarief: {r.effectiefTarief}%</p></div>
