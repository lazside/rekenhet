...
export default function KinderbijslagCalculator() {
  ...
  return (<div className="space-y-6">
    <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm space-y-4">
      <h2 className="text-sm font-semibold text-gray-900 flex items-center gap-2"><Baby className="h-4 w-4 text-indigo-600" />Kinderbijslag</h2>
      ...
      <button onClick={addK} className="flex items-center gap-1.5 text-sm font-medium text-indigo-600 hover:text-indigo-700"><Plus className="h-4 w-4" /> Kind toevoegen</button>
    </div>
    <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
      <div className="grid grid-cols-2 gap-4"><div className="rounded-xl bg-gradient-to-br from-indigo-500 to-indigo-700 p-4 text-white text-center shadow-sm"><p className="text-xs text-indigo-200">Per maand</p><p className="text-2xl font-bold tabular-nums mt-1">€{r.totaalPerMaand.toFixed(2)}</p></div><div className="rounded-xl bg-emerald-500 to-emerald-700 bg-gradient-to-br p-4 text-white text-center shadow-sm"><p className="text-xs text-emerald-200">Per kwartaal</p><p className="text-2xl font-bold tabular-nums mt-1">€{r.totaalPerKwartaal.toFixed(2)}</p></div></div>
    </div>
