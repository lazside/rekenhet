...
export default function CalculatorHub() {
  ...
  if (!config) {
    return (
      <div className="rounded-xl border border-gray-200 bg-white p-6 text-center text-sm text-gray-400 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-500">
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm space-y-4 dark:border-slate-700 dark:bg-slate-900">
        ...
        <p className="text-xs text-gray-500 dark:text-slate-400 -mt-2">{config.description}</p>
          ...
          {config.fields.map((f, idx) => {
            ...
            return (
              <div key={f.key} className={`space-y-1 ${spanClass}`}>
                <label className="text-xs font-medium text-gray-500 dark:text-slate-400">{f.label}</label>
                {f.type === "select" ? (
                  <select value={String(values[f.key] ?? f.default)} onChange={(e) => setValues((p) => ({ ...p, [f.key]: e.target.value }))} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm bg-white dark:border-slate-600 dark:bg-slate-800 dark:text-slate-200">
                    {f.options?.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
                  </select>
                ) : f.type === "date" ? (
                  <input type="date" value={String(values[f.key] ?? f.default)} onChange={(e) => setValues((p) => ({ ...p, [f.key]: e.target.value }))} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm dark:border-slate-600 dark:bg-slate-800 dark:text-slate-200" />
                ) : (
                  <input type="number" value={Number(values[f.key] ?? f.default)} onChange={(e) => setValues((p) => ({ ...p, [f.key]: parseFloat(e.target.value) || 0 }))} min={f.min} max={f.max} step={f.step ?? 1} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm dark:border-slate-600 dark:bg-slate-800 dark:text-slate-200 [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none" />
                )}
              </div>
            );
          })}
        ...
        <div className="grid grid-cols-2 gap-3">
          {config.outputs.map((out) => {
            ...
            return isPrimary ? (
              ...
              <div key={out.key} className="rounded-lg bg-gray-50 p-3 text-center border border-gray-100 dark:bg-slate-800 dark:border-slate-700">
                <p className="text-[10px] text-gray-500 dark:text-slate-400">{out.label}</p>
                <p className="text-sm font-bold text-gray-700 tabular-nums dark:text-slate-200">
                  {display}
