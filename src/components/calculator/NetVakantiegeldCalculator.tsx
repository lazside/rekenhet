...
export default function NetVakantiegeldCalculator() {
  ...
  return (<div className="space-y-6"><div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm space-y-5">
    <h2 className="text-sm font-semibold text-gray-900 flex items-center gap-2"><Umbrella className="h-4 w-4 text-indigo-600" />Netto Vakantiegeld</h2>
    <div className="space-y-1.5"><label className="text-sm font-medium text-gray-700">Bruto jaarsalaris (€)</label><input type="text" value={sal} onChange={e=>setSal(+e.target.value||0)} className="input-base"/></div>
    <div className="flex gap-2"><button onClick={()=>setAhk(!ahk)} className={`flex-1 rounded-lg px-3 py-2 text-sm font-medium transition-all ${ahk?"bg-indigo-600 text-white shadow-sm":"bg-gray-100 text-gray-600"}`}>AHK: {ahk?"Aan":"Uit"}</button><button onClick={()=>setAk(!ak)} className={`flex-1 rounded-lg px-3 py-2 text-sm font-medium transition-all ${ak?"bg-indigo-600 text-white shadow-sm":"bg-gray-100 text-gray-600"}`}>Arbeidskorting: {ak?"Aan":"Uit"}</button></div>
  </div>
  <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm space-y-3">
    <div className="grid grid-cols-2 gap-3">
      ...
      <div className="rounded-xl bg-gradient-to-br from-indigo-500 to-indigo-700 p-4 text-white text-center"><p className="text-xs text-indigo-200">Netto</p><p className="text-xl font-bold tabular-nums mt-1">{formatEUR(r.nettoVakantiegeld)}</p></div>
    </div>
