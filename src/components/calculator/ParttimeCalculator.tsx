...
export default function ParttimeCalculator() {
  ...
  return (<div className="space-y-6"><div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm space-y-5">
    <h2 className="text-sm font-semibold text-gray-900 flex items-center gap-2"><ArrowRightLeft className="h-4 w-4 text-indigo-600" />Parttime Factor</h2>
    <div className="grid grid-cols-2 gap-4"><div className="space-y-1.5"><label className="text-sm font-medium text-gray-700">Fulltime salaris (€)</label><input type="text" value={full} onChange={e=>setFull(+e.target.value||0)} className="input-base"/></div><div className="space-y-1.5"><label className="text-sm font-medium text-gray-700">Fulltime uren</label><input type="text" value={hFull} onChange={e=>setHFull(+e.target.value||0)} className="input-base"/></div></div>
    <div className="space-y-1.5"><label className="text-sm font-medium text-gray-700">Parttime uren</label><div className="relative"><input type="range" min={1} max={hFull} value={hPart} onChange={e=>setHPart(+e.target.value)} className="w-full accent-indigo-600"/><div className="flex justify-between text-xs text-gray-400 mt-1"><span>1</span><span className="font-semibold text-indigo-600">{hPart} uur</span><span>{hFull}</span></div></div></div>
  </div>
  <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm space-y-3">
    <div className="text-center py-4"><p className="text-xs text-gray-500 mb-1">Parttime salaris</p><p className="text-4xl font-bold text-indigo-600 tabular-nums">{formatEUR(r.parttimeSalary)}</p></div>
