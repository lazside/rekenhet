    ...
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm space-y-5">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          <div className="space-y-1.5"><label className="text-xs font-medium text-gray-500">Hypotheek (€)</label><input type="number" value={h} onChange={e => setH(+e.target.value||0)} className="w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm" /></div>
          <div className="space-y-1.5"><label className="text-xs font-medium text-gray-500">Rente (%)</label><input type="number" step="0.1" value={r} onChange={e => setR(+e.target.value||0)} className="w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm" /></div>
          <div className="space-y-1.5"><label className="text-xs font-medium text-gray-500">Looptijd (jr)</label><input type="number" value={l} onChange={e => setL(+e.target.value||0)} className="w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm" /></div>
          <div className="space-y-1.5"><label className="text-xs font-medium text-gray-500">Extra aflossing (€)</label><input type="number" value={e} onChange={e => setE(+e.target.value||0)} className="w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm" /></div>
          <div className="space-y-1.5"><label className="text-xs font-medium text-gray-500">In jaar</label><input type="number" min={1} max={l} value={sj} onChange={e => setSj(Math.min(l, Math.max(1, +e.target.value||1)))} className="w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm" /></div>
        </div>
      </div>
