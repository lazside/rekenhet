...
export default function BtwSimpleCalculator() {
  ...
  return (<div className="space-y-6"><div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm space-y-5">
    <h2 className="text-sm font-semibold text-gray-900 flex items-center gap-2"><Percent className="h-4 w-4 text-indigo-600" />BTW Berekenen</h2>
    <div className="space-y-1.5"><label className="text-sm font-medium text-gray-700">Bedrag</label><input type="text" step="0.01" value={b} onChange={e=>setB(+e.target.value||0)} className="input-base"/></div>
    <div className="grid grid-cols-3 gap-2">{[21,9,0].map(v=><button key={v} onClick={()=>setT(v)} className={`rounded-lg px-3 py-2.5 text-sm font-medium transition-all ${t===v?"bg-indigo-600 text-white shadow-sm":"bg-gray-100 text-gray-600 hover:bg-gray-200"}`}>{v}%</button>)}</div>
    <div className="flex gap-2"><button onClick={()=>setIncl(true)} className={`flex-1 rounded-lg px-3 py-2 text-sm font-medium transition-all ${incl?"bg-indigo-600 text-white shadow-sm":"bg-gray-100 text-gray-600 hover:bg-gray-200"}`}>Incl. BTW</button><button onClick={()=>setIncl(false)} className={`flex-1 rounded-lg px-3 py-2 text-sm font-medium transition-all ${!incl?"bg-indigo-600 text-white shadow-sm":"bg-gray-100 text-gray-600 hover:bg-gray-200"}`}>Excl. BTW</button></div>
  </div>
