...
export default function UurtariefCalculator() {
  ...
  return (<div className="space-y-6"><div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm space-y-5">
    <h2 className="text-sm font-semibold text-gray-900 flex items-center gap-2"><Clock className="h-4 w-4 text-indigo-600" />Uurtarief</h2>
    <div className="grid grid-cols-2 gap-4"><div className="space-y-1.5"><label className="text-sm font-medium text-gray-700">Uurtarief (€)</label><input type="text" value={rate} onChange={e=>setRate(+e.target.value||0)} className="input-base"/></div><div className="space-y-1.5"><label className="text-sm font-medium text-gray-700">Uren per week</label><input type="text" value={hours} onChange={e=>setHours(+e.target.value||0)} className="input-base"/></div></div>
  </div>
  <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm space-y-3">
    <h2 className="text-sm font-semibold text-gray-900 flex items-center gap-2"><Euro className="h-4 w-4 text-indigo-600" />Inkomen</h2>
    <div className="rounded-xl bg-gradient-to-br from-indigo-600 to-indigo-800 p-5 text-white shadow-lg text-center"><p className="text-sm text-indigo-200">Netto per maand</p><p className="text-4xl font-bold tabular-nums mt-1">{formatEUR(r.nettoMaand)}</p></div>
