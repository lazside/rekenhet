...
export default function SpeedingFineCalculator() {
  ...
  return (<div className="space-y-6"><div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm space-y-5">
    <h2 className="text-sm font-semibold text-gray-900 flex items-center gap-2"><Gauge className="h-4 w-4 text-indigo-600" />Snelheid</h2>
    <div className="grid grid-cols-2 gap-4">
      <div className="space-y-1.5"><label className="text-sm font-medium text-gray-700">Jouw snelheid (km/h)</label><input type="text" value={speed} onChange={e=>setSpeed(+e.target.value||0)} className="input-base"/></div>
      <div className="space-y-1.5"><label className="text-sm font-medium text-gray-700">Toegestane snelheid (km/h)</label><input type="text" value={limit} onChange={e=>setLimit(+e.target.value||0)} className="input-base"/></div>
    </div>
    <div className="flex gap-2"><button onClick={()=>setMotorway(true)} className={`flex-1 rounded-lg px-3 py-2 text-sm font-medium transition-all ${motorway?"bg-indigo-600 text-white shadow-sm":"bg-gray-100 text-gray-600 hover:bg-gray-200"}`}>Snelweg</button><button onClick={()=>setMotorway(false)} className={`flex-1 rounded-lg px-3 py-2 text-sm font-medium transition-all ${!motorway?"bg-indigo-600 text-white shadow-sm":"bg-gray-100 text-gray-600 hover:bg-gray-200"}`}>Binnenweg</button></div>
  </div>
  <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm space-y-3">
    <h2 className="text-sm font-semibold text-gray-900 flex items-center gap-2"><Euro className="h-4 w-4 text-indigo-600" />Boete</h2>
