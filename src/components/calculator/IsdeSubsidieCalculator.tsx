...
export default function IsdeSubsidieCalculator() {
  ...
  return (<div className="space-y-6"><div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm space-y-5">
    ...
    <div className="grid grid-cols-2 gap-4">
      <div className="space-y-2"><label className="text-xs font-medium text-gray-500">Warmtepomp type</label><select value={wt} onChange={e=>setWt(e.target.value as WarmtepompType)} className="input-base"><option value="hybride">Hybride warmtepomp</option><option value="volledig">Volledig elektrisch</option></select><div className="space-y-1"><label className="text-xs text-gray-500">Vermogen: {kw} kW</label><input type="range" min={2} max={16} value={kw} onChange={e=>setKw(+e.target.value)} className="w-full accent-green-600"/></div></div>
      <div className="space-y-2"><label className="text-xs font-medium text-gray-500">Isolatie type</label><select value={it} onChange={e=>setIt(e.target.value as IsolatieType)} className="input-base"><option value="hr++">HR++ glas</option><option value="dak">Dakisolatie</option><option value="vloer">Vloerisolatie</option><option value="spouw">Spouwmuurisolatie</option></select><div className="space-y-1"><label className="text-xs text-gray-500">Oppervlak: {m2} m²</label><input type="range" min={5} max={200} value={m2} onChange={e=>setM2(+e.target.value)} className="w-full accent-green-600"/></div></div>
    </div>
  </div>
