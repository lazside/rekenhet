...
export default function FireCalculator() {
  ...
  return (<div className="space-y-6"><div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm space-y-5">
    <h2 className="text-sm font-semibold text-gray-900 flex items-center gap-2"><TrendingUp className="h-4 w-4 text-indigo-600" />FIRE Calculator</h2>
    <div className="grid grid-cols-2 gap-4">
      <div className="space-y-1.5"><label className="text-sm font-medium text-gray-700">Huidig vermogen (€)</label><input type="text" value={v} onChange={e=>setV(+e.target.value||0)} className="input-base"/></div>
      <div className="space-y-1.5"><label className="text-sm font-medium text-gray-700">Maandelijkse inleg (€)</label><input type="text" value={m} onChange={e=>setM(+e.target.value||0)} className="input-base"/></div>
      <div className="space-y-1.5"><label className="text-sm font-medium text-gray-700">Rendement p/j (%)</label><input type="text" step="0.5" value={r} onChange={e=>setR(+e.target.value||0)} className="input-base"/></div>
      <div className="space-y-1.5"><label className="text-sm font-medium text-gray-700">Inflatie (%)</label><input type="text" step="0.5" value={i} onChange={e=>setI(+e.target.value||0)} className="input-base"/></div>
      <div className="space-y-1.5"><label className="text-sm font-medium text-gray-700">Jaarkosten pensioen (€)</label><input type="text" value={k} onChange={e=>setK(+e.target.value||0)} className="input-base"/></div>
      <div className="space-y-1.5"><label className="text-sm font-medium text-gray-700">Veilige opname (%)</label><input type="text" step="0.5" value={o} onChange={e=>setO(+e.target.value||0)} className="input-base"/></div>
    </div>
  </div>
  <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
    ...
    <div className="grid grid-cols-2 gap-3 mt-3">
      <div className="rounded-lg bg-blue-50 p-3 text-center border border-blue-100"><p className="text-[10px] text-indigo-600">FIRE-getal</p><p className="text-sm font-bold text-blue-700 tabular-nums">{formatEUR(res.fireGetal)}</p></div>
