...
export default function HuurtoeslagCalculator() {
 ...
 <input type="number" value={kaleHuur} onChange={(e) => setKaleHuur(parseFloat(e.target.value) || 0)} min={0} max={1200} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm [appearance:textfield]" />
 </div>
 ...
 <input type="number" value={serviceKosten} onChange={(e) => setServiceKosten(parseFloat(e.target.value) || 0)} min={0} max={500} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm [appearance:textfield]" />
 </div>
 ...
 <input type="number" value={inkomen} onChange={(e) => setInkomen(parseFloat(e.target.value) || 0)} min={0} max={200000} step={500} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm [appearance:textfield]" />
 </div>
 ...
 <input type="number" value={vermogen} onChange={(e) => setVermogen(parseFloat(e.target.value) || 0)} min={0} max={500000} step={1000} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm [appearance:textfield]" />
 </div>

 {/* Leeftijd */}
 <div className="space-y-1">
 <label className="text-xs font-medium text-gray-500">Leeftijd</label>
 <select value={leeftijd} onChange={(e) => setLeeftijd(e.target.value as LeeftijdGroep)} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm bg-white">
 ...
 <select value={huishouden} onChange={(e) => setHuishouden(e.target.value as Huishouden)} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm bg-white">
 ...
 <input type="number" value={medebewonerBijdrage} onChange={(e) => setMedebewonerBijdrage(parseFloat(e.target.value) || 0)} min={0} max={1000} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm [appearance:textfield]" />
 </div>
 )}
 </div>
 </div>
 ...
 <p className="text-[10px] text-gray-500">Rekenhuur</p>
 <p className="text-sm font-bold text-gray-700 tabular-nums">{formatEur(result.rekenhuurPerMaand)}</p>
 </div>
 <div className="rounded-lg bg-gray-50 p-3 text-center border border-gray-100">
 <p className="text-[10px] text-gray-500">Drempelhuur</p>
 <p className="text-sm font-bold text-gray-700 tabular-nums">{formatEur(result.drempelhuurPerMaand)}</p>
 </div>
 <div className="rounded-lg bg-teal-50 p-3 text-center border border-teal-100">
 <p className="text-[10px] text-teal-600">Basishuur</p>
 <p className="text-sm font-bold text-teal-700 tabular-nums">{formatEur(result.basishuurPerMaand)}</p>
 </div>
 <div className="rounded-lg bg-gray-50 p-3 text-center border border-gray-100">
 <p className="text-[10px] text-gray-500">Dekking</p>
 <p className="text-sm font-bold text-gray-700 tabular-nums">{result.dekkingsPercentage}%</p>
 </div>
 </div>
 </>
 ) : (
 ...
 <p><strong className="text-gray-700">1. Rekenhuur</strong> = kale huur + servicekosten (max {formatEur(MAXIMALE_HUURGRENS)})</p>
 <p><strong className="text-gray-700">2. Drempelhuur</strong> = inkomensafhankelijk deel ({Math.round(DREMPELPCT_ALLEENSTAAND * 100)}% voor alleenstaand, {Math.round(DREMPELPCT_MEERPERSOONS * 100)}% voor meerpersoons)</p>
 <p><strong className="text-gray-700">3. Basishuur</strong> = rekenhuur − drempelhuur (dit wordt gesubsidieerd)</p>
 <p><strong className="text-gray-700">4. Toeslag</strong> = {TOESLAG_PERCENTAGE * 100}% van de basishuur (binnen de aftoppingsgrens)</p>
 </div>
 </div>

 {/* ── Disclaimer ── */}
 <div className="flex items-start gap-2 rounded-lg border border-slate-200 bg-slate-50/50 p-3" role="note" aria-label="Juridische disclaimer">
 ...
 <strong className="font-medium text-slate-600">Disclaimer:</strong>{""}
