...
export default function CompoundInterestCalculator() {
  ...
  return (<div className="space-y-6"><div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm space-y-5">
    <h2 className="text-sm font-semibold text-gray-900 flex items-center gap-2"><TrendingUp className="h-4 w-4 text-indigo-600" />Vermogensgroei</h2>
    <div className="grid grid-cols-2 gap-4">
      <div className="space-y-1.5"><label className="text-sm font-medium text-gray-700">Startkapitaal (€)</label><input type="text" value={p} onChange={e=>setP(+e.target.value||0)} className="input-base"/></div>
      <div className="space-y-1.5"><label className="text-sm font-medium text-gray-700">Jaarlijks rendement (%)</label><input type="text" step="0.5" value={r} onChange={e=>setR(+e.target.value||0)} className="input-base"/></div>
      <div className="space-y-1.5"><label className="text-sm font-medium text-gray-700">Looptijd (jaren)</label><input type="text" value={y} onChange={e=>setY(+e.target.value||0)} className="input-base"/></div>
      <div className="space-y-1.5"><label className="text-sm font-medium text-gray-700">Maandelijks inleggen (€)</label><input type="text" value={m} onChange={e=>setM(+e.target.value||0)} className="input-base"/></div>
    </div>
  </div>
