import { registerPages } from "../types";

registerPages({
  slug: "ideale-lengte-kind",
  seo: {
    title: "Ideale Lengte Kind — Groeicurve | Rekenhet.nl",
    description: "Gratis kinderlengte calculator met gemiddelde lengte per leeftijd.",
    h1: "Ideale Lengte Kind — Direct inzicht in je gezondheid",
    intro: "Een gezonde leefstijl begint met inzicht. Deze Ideale Lengte Kind geeft je direct duidelijkheid. Bekijk de gemiddelde lengte voor kinderen op basis van leeftijd. Het is een handig hulpmiddel om je gezondheid te monitoren en doelen te stellen. Gebruik de resultaten als richtlijn en raadpleeg bij twijfel een arts of specialist.",
    conclusion: "Deze calculator geeft een indicatie op basis van wetenschappelijke formules en gemiddelden. Het is geen medisch advies en vervangt geen consult bij een arts, diëtist of specialist. Gebruik de inzichten als leidraad voor een gezonde leefstijl en raadpleeg bij vragen altijd een deskundige.",
    keywords: ["lengte kind","groeicurve","gemiddelde lengte","kind","ideale lengte kind","ideale lengte kind 2026"],
  },
  calculator: {
    componentSlug: "ideale-lengte-kind",
    categorySlug: "gezondheid",
  },
  categoryLabel: "Gezondheid",
    faqs: [
    {
      question: "Hoe nauwkeurig is de Ideale Lengte Kind?",
      answer: "Bekijk de gemiddelde lengte voor kinderen op basis van leeftijd De calculator geeft een betrouwbare indicatie op basis van wetenschappelijke formules en standaardwaarden.",
    },
    {
      question: "Is de Ideale Lengte Kind een vervanging voor medisch advies?",
      answer: "Nee, deze calculator is bedoeld als informatief hulpmiddel en vervangt geen medisch consult. Raadpleeg altijd een arts of specialist voor persoonlijk gezondheidsadvies.",
    },
    {
      question: "Welke formule gebruikt de Ideale Lengte Kind?",
      answer: "De calculator gebruikt algemeen geaccepteerde wetenschappelijke formules en standaardwaarden. De exacte formule wordt bij de resultaten getoond.",
    },
    {
      question: "Voor welke leeftijden is de Ideale Lengte Kind geschikt?",
      answer: "De calculator is geschikt voor volwassenen. Voor kinderen en jongeren tot 18 jaar kunnen afwijkende waarden en formules gelden.",
    }
  ],
}
);
